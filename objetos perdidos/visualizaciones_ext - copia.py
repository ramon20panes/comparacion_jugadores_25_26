...
def plot_pass_network_for_player(
    ...
    *,
    ...
    team_id=None,
    team_color="#00E5FF",
    show_all=True,                # <- lo haremos funcionar
    min_edge_count=1,
    ...
    for _, r in avg_pos.iterrows():
        ...
        if p == pid:
            # Si nos pasan un nick explícito (nick_name), úsalo; si no, usa el nombre que tengamos
            ...
            # Color del texto del destacado: gris claro si te lo pasan, si no el color de equipo
...
def plot_pass_network_for_player_auto(
    ...
    **kwargs
    ...
    # Si no nos pasan color: intenta resolver por teamId del propio jugador
...
def draw_winger_legend(
    ...
    *,
    x=7,           # columna base
    y0=25,          # fila inicial
    dy=6,         # salto vertical entre elementos
    fz=10,           # tamaño de texto
    L=10,           # longitud de TODAS las flechas
    sym_s=42,       # tamaño de símbolos (círculo/rombo)
    sym_lw=1.8,     # grosor del borde de símbolos
    head=14,        # tamaño de la punta de flecha
    lw_goal=2.6,    # grosor de la flecha de GOL (más ancha)
    lw_ot=1.6,      # grosor de Tiro a puerta
    lw_off=1.6,     # grosor de Tiro fuera
    lw_ast=2.2,     # grosor de Asistencia
    lw_kp=1.6,      # grosor de Pase clave
    lw_cross=1.2    # grosor de Centro
):
    """
    Leyenda compacta: flechas de igual longitud y símbolos centrados en la mitad
    de la flecha. Coordenadas Opta (0..100).
    """
    col = lambda k: EVENT_COLORS[k]["edge"]

    x0  = x           # origen de flechas
    x1  = x0 + L      # final de flechas (todas iguales)
    xmid = x0 + L/2   # mitad: aquí van los símbolos
    tx  = x1 + 3      # columna de texto
    y   = y0

    # helper de flecha horizontal
    ...
    # --- TIROS / GOL ----------------------------------------------------------
    _flecha("Gol",           "goal",     lw_goal, m=head+2)   # más ancho y cabeza mayor
    _flecha("Tiro a puerta", "shot_ot",  lw_ot)
    _flecha("Tiro fuera",    "shot_off", lw_off)

    # --- PASES ----------------------------------------------------------------
    _flecha("Asistencia",    "assist",   lw_ast)
    _flecha("Pase clave",    "key_pass", lw_kp)
    _flecha("Centros",       "cross",    lw_cross)

    # --- REGATES (símbolos centrados en la mitad de la flecha) ----------------
...
def plot_winger_actions_for_player(
    ...
    *,
    ...
    d["gmz"] = to_num(df_events.get("goalMouthZ", df_events.get("qNone_GoalMouthZ", np.nan)))  # por si te sirve luego
    ...
    if "eid" in d.columns:
        ...
        # (Si en tu feed existe un "shot" separado del "goal" conectado por relatedEventId,
        #  esto ya evita pintar doble. En tu muestra, el gol viene como fila propia con endX/Y.)
    ...
    # Pases: asistencia / key pass / centros
    draw_arrows(ASSIST,   lw=1.8, ls="-",  col=EVENT_COLORS["assist"]["edge"])
    draw_arrows(KEY_PASS, lw=1.2, ls="-",  col=EVENT_COLORS["key_pass"]["edge"])
    draw_arrows(CROSS_ANY,lw=1.2, ls="-",  col=EVENT_COLORS["cross"]["edge"])
    ...
    def _endpoint_for_shot(r):
        ...
        # c) proyección lineal hacia x=100 (si tuvieras ey/endY parcial)
        if pd.notna(ex) and pd.notna(ey):
...
def plot_winger_actions_for_player_auto(
    ...
    """
    # este helper ya lo tienes definido en tu flujo
    dfe, dfp = get_match_data_for_player(player_id, players_files, events_files)
    if dfe is None:
        return
